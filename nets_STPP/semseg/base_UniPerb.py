from nets_STPP.backbone.resnet import resnet50, resnet101
...
class BaseNet(nn.Module):
    ...
    def base_forward(self, x):
        print('inside basenet base_forward')
        h, w = x.shape[-2:]
    ...
    def forward(self, x, tta=False, need_fp=False):
        if not tta:
            # print('inside basenet forward')
            return self.base_forward(x, need_fp=False)
