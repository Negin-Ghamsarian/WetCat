from nets_STPP.semseg.base import BaseNet
...
class DeepLabV3Plus(BaseNet):
    def __init__(self, backbone, nclass, rep_head = False, input_size = [512,512]):
        super(DeepLabV3Plus, self).__init__(backbone)

        backbone_size = (input_size[0]//16)*(input_size[1]//16)
        ...
        if self.rep_head:
            self.representation = nn.Sequential(
                    nn.Conv2d(high_level_channels, 8, kernel_size=1, stride=1, padding=0, bias=True),
                    nn.Flatten(),
    ...
    def base_forward(self, x):
        ...
        if self.rep_head:
            out_rep = self.representation(c4)
        else:
            out_rep = 0    
        return out, out_rep    
