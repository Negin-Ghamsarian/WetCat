...
class SequenceModel(nn.Module):
    def __init__(self, num_classes):
        ...
        self.bigru = nn.GRU(input_size=1024, hidden_size=256, num_layers=1, batch_first=True, bidirectional=True)
...
if __name__ == '__main__':
    ...
    cnn_GRU = CNN_BiGRU(4, 10,1024).to(device)
    video_input = torch.zeros(16, 10, 3, 256, 256).to(device)
    video_output = cnn_GRU(video_input)
